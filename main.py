...
def is_off_screen(self):
    if self.x < 0 or self.x + self.width > screen.get_width() or self.y < 0 or self.y + self.height > screen.get_height():
      return True
    else:
      return False
...
while running:
  ...
  if not game_over:
    ...
    screen.blit(PLAYER_SPRITE, (player["x"] - player["width"] / 2, player["y"] - player["height"] / 2))
    ...
    for e in enemy_list:
      ...
      if bullet_list.__len__() < 0: break
      for b in bullet_list:
        if pygame.Rect.colliderect(enemy_rect, pygame.Rect(b.x, b.y, b.width, b.height)):
          bullet_list.remove(b)
          e.health -= b.damage
          if e.health > 0: break
          enemy_list.remove(e)
    ...
    pygame.draw.rect(screen, "green", pygame.Rect(5, 5, (200 / PLAYER_HEALTH) * player["health"], 20))
